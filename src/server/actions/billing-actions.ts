...
export async function startCheckoutAction(_prev: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    const result = await startCheckout(session.user.id, readString(formData, "weddingId"), { kind, code });
    if (!result.ok) return { status: "error", message: CHECKOUT_ERRORS[result.reason] };
